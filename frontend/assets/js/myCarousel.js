(function ($) {
    $.fn.myCarousel = function (options) {
        ...
        this.each(function () {
            ...
            _this.find(options.sameNode).width(_this.width()).height(_this.height());   //设置图片宽高
            ...
            _this.mouseover(function (event) {  //鼠标在上边的时候停止运动
                options.leftBtn.fadeIn();
                options.rightBtn.fadeIn();
                /* Act on the event */
                clearInterval(time);
            }).mouseleave(function (event) { //离开继续
                options.leftBtn.fadeOut();
                options.rightBtn.fadeOut();
                /* Act on the event */
                if (options.switcHover) {
                    options.loopNode.html("");
//                    $(options.sameNode).each(function(index, elem){
//                        options.loopNode.append(elem);
//                    });
                    for (var i = 0; i < oLen; i++) {
                        options.loopNode.append("<img src='/filedata/site/image/banner_1_" + ((i + page) % oLen + 1) + ".jpg' name='" + (i + page) % oLen + "' alt=''  width='" + _this.width() + "' height='" + _this.height() + "'/>");
                    }
                }
                ;
